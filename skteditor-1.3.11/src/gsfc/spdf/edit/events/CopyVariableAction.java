/*
 * NOSA HEADER START
 *
 * The contents of this file are subject to the terms of the NASA Open 
 * Source Agreement (NOSA), Version 1.3 only (the "Agreement").  You may 
 * not use this file except in compliance with the Agreement.
 *
 * You can obtain a copy of the agreement at
 *   docs/NASA_Open_Source_Agreement_1.3.txt
 * or 
 *   https://spdf.gsfc.nasa.gov/skteditor/NASA_Open_Source_Agreement_1.3.txt
 *
 * See the Agreement for the specific language governing permissions
 * and limitations under the Agreement.
 *
 * When distributing Covered Code, include this NOSA HEADER in each
 * file and include the Agreement file at 
 * docs/NASA_Open_Source_Agreement_1.3.txt.  If applicable, add the 
 * following below this NOSA HEADER, with the fields enclosed by 
 * brackets "[]" replaced with your own identifying information: 
 * Portions Copyright [yyyy] [name of copyright owner]
 *
 * NOSA HEADER END
 *
 * Copyright (c) 2011-2022 United States Government as represented by 
 * the National Aeronautics and Space Administration. No copyright is 
 * claimed in the United States under Title 17, U.S.Code. All Other 
 * Rights Reserved.
 *
 * $Id: CopyVariableAction.java,v 1.14 2022/03/24 10:38:28 btharris Exp $
 */

//$Id: CopyVariableAction.java,v 1.14 2022/03/24 10:38:28 btharris Exp $
package gsfc.spdf.edit.events;

import javax.swing.*;

import java.awt.Cursor;
import java.awt.event.*;
import java.lang.*;
import java.util.Vector;
import java.util.Enumeration;

import gsfc.nssdc.cdf.*;
import gsfc.spdf.edit.guis.*;

/**
 * Copy the currently selected variable.  The user will be prompted for a 
 * new variable name.
 *
 * Note:  This will copy the metadata as well as any data for the selected
 * variable.
 *
 * @author Phil Williams
 * @version $Revision: 1.14 $
 */

public class CopyVariableAction extends AbstractAction {
    
    private static final String DEFAULT_NAME = "Copy";

    private static final String DEFAULT_ICON_NAME = "Copy16.gif";

    private static Icon DEFAULT_ICON;

    private SKTEditor myEditor;

    public CopyVariableAction(SKTEditor myEditor) {
	this(myEditor, DEFAULT_NAME, DEFAULT_ICON);
    }

    public CopyVariableAction(SKTEditor myEditor, String name, Icon icon) {
	super(name, icon);
	this.myEditor = myEditor;
    }

    public void putValue(String key, Object value) {
	if (value != null)
	    super.putValue(key, value);
    }

    public void actionPerformed(ActionEvent event) {
	SKTEditor.setWaitCursor();
	Object vObj = myEditor.variablePanel.getSelectedVar();
	if (vObj == null)
	    JOptionPane.showMessageDialog(SKTEditor.edit, 
					  "No variable selected",
					  "SKTEditor: Error",
					  JOptionPane.ERROR_MESSAGE);
	else {

            Variable oldselVar = (Variable)vObj;
            gsfc.spdf.istp.Variable selVar = new gsfc.spdf.istp.Variable(oldselVar);
            String message = 
                "Copy '" + selVar.getName() + "'\n" +
                "variable to a new variable.\n\n" +
                "Enter a name for the new variable:";

	    String dest = 
		JOptionPane.showInputDialog(SKTEditor.edit,
                                            message,
					    "Copy Variable",
					    JOptionPane.QUESTION_MESSAGE);
	    if (dest != null) {
		try {

                    myEditor.variablePanel.saveVariableChanges();

		    gsfc.spdf.istp.Variable dv = selVar.duplicate(dest);                    
		    myEditor.variablePanel.
			addToListOfVariables(dv.getName());
                    myEditor.variablePanel.updateVarPanel(dv.getCdfVariable());
                    myEditor.variablePanel.reselectCurrentVariable();
                  
		} 
                catch (CDFException ce) {
		    JOptionPane.
			showMessageDialog(SKTEditor.edit, 
					  "CDF Error:\n"+
					  ce.getMessage(),
					  "CDFException",
					  JOptionPane.ERROR_MESSAGE);
		}
	    }
	}
	SKTEditor.setDefaultCursor();
    }
    
    static {
	String iconPath = SKTEditor.propertyPath +
	    SKTEditor.appProperties.getProperty("icon.path");
        try {
	    DEFAULT_ICON = new ImageIcon(
                                  CopyVariableAction.class.getResource(
                                              iconPath + DEFAULT_ICON_NAME));
        }
        catch(NullPointerException e) {
            System.err.println("cannot find icon " + iconPath +
                               DEFAULT_ICON_NAME + " -- continuing without it");
        };
    }
    
    
    
 

} // CopyVariableAction
